...
def create_template_ref(template, row, df, rownum):
    for col in df.columns:
        if "PLACEREF" in col and pd.notna(row[col]):
            place_ref = ET.SubElement(template, "placeRef", id=f"def{col}{rownum}", ref=nan_to_empty_string(row[col]))
        if "TEMPLATEREF" in col and pd.notna(row[col]):
            template_ref = ET.SubElement(template, "templateRef", id=f"def{col}{rownum}", ref=nan_to_empty_string(row[col]))
...
print("XMLファイルが生成されました: "+ filePath().OUTPUT_FILE_PATH)
